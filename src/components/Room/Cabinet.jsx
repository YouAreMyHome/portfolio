...
function Cabinet() {
  return (
    <group position={[3.2, 0, -3.2]}>
      ...
      {/* Cabinet top */}
      <mesh receiveShadow position={[0, 1.04, 0]}>
      ...
      <group position={[0, 1.07, 0]}>
        {/* Small plant */}
        <mesh receiveShadow position={[-0.25, 0.06, 0]}>
          <cylinderGeometry args={[0.05, 0.04, 0.1, 6]} />
          <meshToonMaterial color="#D2691E" />
        </mesh>
        <mesh receiveShadow position={[-0.25, 0.14, 0]}>
          <sphereGeometry args={[0.06, 8, 8]} />
          <meshToonMaterial color={COLORS.plant} />
        </mesh>
        
        {/* Photo frame */}
        <mesh receiveShadow position={[0.2, 0.08, 0]} rotation={[0, 0.2, 0]}>
          <boxGeometry args={[0.12, 0.15, 0.02]} />
          <meshToonMaterial color="#5A4A3A" />
        </mesh>
        <mesh receiveShadow position={[0.2, 0.08, 0.012]} rotation={[0, 0.2, 0]}>
      ...
      {[[-0.35, -0.2], [0.35, -0.2], [-0.35, 0.2], [0.35, 0.2]].map(([x, z], i) => (
        <mesh receiveShadow key={i} position={[x, 0.03, z]}>
