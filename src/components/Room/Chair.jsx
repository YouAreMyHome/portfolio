    ...
    >
      ...
      <group position={[0, 0, 0]}>
        ...
        <group position={[0, 0.48, 0]}>
            {/* Đệm chính */}
            <RoundedBox receiveShadow args={[0.5, 0.08, 0.52]} radius={0.02}>
                <meshStandardMaterial color={GAMING_COLORS.primary} roughness={0.7} />
            </RoundedBox>
            
            {/* Viền đệm 2 bên (Side Bolsters) - Tạo cảm giác ghế đua */}
            <mesh receiveShadow position={[-0.22, 0.04, 0]} rotation={[0, 0, 0.2]}>
        ...
        <group position={[0, 0.85, -0.22]} rotation={[-0.1, 0, 0]}> {/* Hơi ngả ra sau */}
            {/* Lưng chính */}
            <RoundedBox receiveShadow args={[0.45, 0.8, 0.06]} radius={0.02}>
                 <meshStandardMaterial color={GAMING_COLORS.primary} />
            </RoundedBox>
            
            {/* Cánh ôm lưng (Wings) - Phần vai */}
            <group position={[0, 0.2, 0.02]}>
                 <mesh receiveShadow position={[-0.24, 0, 0.05]} rotation={[0, 0.3, 0]}>
            ...
            <mesh receiveShadow position={[0, 0.28, 0.06]}>
                 <boxGeometry args={[0.25, 0.12, 0.06]} />
                 <meshStandardMaterial color={GAMING_COLORS.accent} />
            </mesh>
            {/* Dây đai gối */}
             <mesh receiveShadow position={[0, 0.28, 0.031]}>
                 <boxGeometry args={[0.26, 0.02, 0.01]} />
                 <meshBasicMaterial color="#111" />
            </mesh>

            {/* Gối lưng (Lumbar Support) */}
             <mesh receiveShadow position={[0, -0.15, 0.05]}>
        ...
        {[-0.28, 0.28].map((x, i) => (
            <group key={i} position={[x, 0.65, -0.05]}>
                ...
                {/* Bề mặt tì tay (Mềm) */}
                <RoundedBox receiveShadow args={[0.08, 0.03, 0.24]} radius={0.01}>
      ...
      <group position={[0, 0.08, 0]}>
          {[0, 1, 2, 3, 4].map((i) => (
              <group key={i} rotation={[0, (i / 5) * Math.PI * 2, 0]}>
                  {/* Chân ghế vát cạnh */}
                  <mesh receiveShadow position={[0.15, 0, 0]}>
                  ...
                  <group position={[0.3, -0.05, 0]}>
                       ...
                       {/* Bánh xe đôi */}
                       <mesh receiveShadow position={[0, 0, 0.015]} rotation={[Math.PI/2, 0, 0]}>
                       ...
                       {/* Bóng đổ giả dưới bánh */}
                        <mesh receiveShadow position={[0, -0.024, 0]} rotation={[-Math.PI / 2, 0, 0]}>
